--
-- VB-AC Remastered v4.0 by VisiBait (VB-SCRIPTS)
--
...
if VB_AC.Enable then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if VB_AC.AntiExplosionDamage then
            ...
            if VB_AC.AntiInfiniteStamina then
            ...
            if VB_AC.AntiBlacklistedTasks then
            ...
            if VB_AC.AntiBlacklistedAnims then
                for _,anim in pairs(VB_AC.BlacklistedAnims) do
            ...
            if VB_AC == nil or _evhandler ~= AddEventHandler or _onresstarting ~= "onResourceStarting" or _onresstart ~= "onResourceStart" or _onclresstart ~= "onClientResourceStart" or _antistop ~= VB_AC then
    ...
    Citizen.CreateThread(function()
        while VB_AC.AntiWeaponModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
    ...
    Citizen.CreateThread(function()
        ...
        while VB_AC.AntiCommandInjection do
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
        if VB_AC.AntiPedRevive and VB_AC.UseESX then
            local distance = #(VB_AC.HospitalCoords - vector3(GetEntityCoords(PlayerPedId())))
            if distance < 50 then
    ...
    Citizen.CreateThread(function()
        Citizen.Wait(5000)
        while VB_AC.OnScreenMenuDetection do
            if not ischecking then
                exports['vbac-module2']:requestScreenshot(function(data)
                    Citizen.Wait(1000)
                    SendNUIMessage({
            ...
            Citizen.Wait(VB_AC.OSMDCheckingTime)
        end
    end)
    ...
    RegisterNUICallback('menucheck', function(data)
        if VB_AC.OnScreenMenuDetection then
            if data.text ~= nil then     
                for _, word in pairs(VB_AC.BlacklistedMenuWords) do
